...
const visionAiPageData = JSON.parse(
  readFileSync(join(__dirname, "../src/data/vision-ai-page.json"), "utf8"),
);
...
async function run() {
  ...
  if (first) {
    ...
    console.log("Updated vision-ai page:", updated?.data?.id || idOrDoc);
    return;
  }
  ...
  console.log("Created vision-ai page:", created?.data?.id || created?.data?.documentId);
}

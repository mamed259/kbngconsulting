...
const aboutUsPageData = JSON.parse(
  readFileSync(join(__dirname, "../src/data/about-us-page.json"), "utf8"),
);
...
async function run() {
  ...
  findQuery.set("filters[slug][$eq]", "about-us");
  ...
  if (first) {
    ...
    console.log("Updated about-us page:", updated?.data?.id || idOrDoc);
    return;
  }
  ...
  console.log("Created about-us page:", created?.data?.id || created?.data?.documentId);
}

...
const founderDiagnosticPageData = JSON.parse(
  readFileSync(join(__dirname, "../src/data/founder-diagnostic-page.json"), "utf8"),
);
...
async function run() {
  ...
  findQuery.set("filters[slug][$eq]", "founder-diagnostic");
  findQuery.set("status", "draft");

  const existing = await strapiRequest(`/api/pages?${findQuery.toString()}`);
  const first = existing?.data?.[0];

  const hydrated = await hydrateImageUrls(founderDiagnosticPageData, {
  ...
  if (first) {
    ...
    console.log("Updated founder-diagnostic page:", updated?.data?.id || idOrDoc);
    return;
  }
  ...
  console.log("Created founder-diagnostic page:", created?.data?.id || created?.data?.documentId);
}

...
async function strapiRequest(path, init = {}) {
  ...
  if (!response.ok) {
    if (response.status === 403) {
      throw new Error(
        `${response.status} Forbidden — API token needs Full Access with Article create/update/publish permissions. ` +
...
async function seedOne(article) {
  ...
  if (first) {
    ...
    console.log("Updated article:", article.slug, updated?.data?.id || idOrDoc);
    return;
  }
  ...
  console.log("Created article:", article.slug, created?.data?.id || created?.data?.documentId);
}

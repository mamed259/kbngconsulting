...
const canaryWavesPageData = JSON.parse(
  readFileSync(join(__dirname, "../src/data/canary-waves-page.json"), "utf8"),
);
...
async function run() {
  ...
  findQuery.set("filters[slug][$eq]", "canary-waves");
  findQuery.set("status", "draft");

  const existing = await strapiRequest(`/api/pages?${findQuery.toString()}`);
  const first = existing?.data?.[0];

  const hydrated = await hydrateImageUrls(canaryWavesPageData, {
  ...
  if (first) {
    ...
    console.log("Updated canary-waves page:", updated?.data?.id || idOrDoc);
    return;
  }
  ...
  console.log("Created canary-waves page:", created?.data?.id || created?.data?.documentId);
}

...
const georgiaPageData = JSON.parse(
  readFileSync(join(__dirname, "../src/data/georgia-page.json"), "utf8"),
);
...
async function run() {
  ...
  if (first) {
    ...
    console.log("Updated georgia page:", updated?.data?.id || idOrDoc);
    return;
  }
  ...
  console.log("Created georgia page:", created?.data?.id || created?.data?.documentId);
}

...
const consultingServicesPageData = JSON.parse(
  readFileSync(join(__dirname, "../src/data/consulting-services-page.json"), "utf8"),
);
...
async function run() {
  ...
  findQuery.set("filters[slug][$eq]", "consulting-services");
  findQuery.set("status", "draft");

  const existing = await strapiRequest(`/api/pages?${findQuery.toString()}`);
  const first = existing?.data?.[0];

  const hydrated = await hydrateImageUrls(consultingServicesPageData, {
  ...
  if (first) {
    ...
    console.log("Updated consulting-services page:", updated?.data?.id || idOrDoc);
    return;
  }
  ...
  console.log("Created consulting-services page:", created?.data?.id || created?.data?.documentId);
}
